    ...
    return (
        <>
            ...
            <div style={{ marginLeft: '2%', marginRight: '2%', marginTop: '1.5%' }}>
                <Button id="upper-add-button" onClick={showDialog} variant='contained'><UploadIcon /> Upload</Button>
                <Dropzone open={dialogOpen} close={closeDialog} username={props.username} snackbar={showSnackbar} />
                <Row className='image-grid' style={{ marginTop: '3%' }}>
                    {imageList.length !== 0 ? (
                        <>
                            {imageList.map((item, index) => (
                                <Col className='image-container' xs={4} sm={3} md={3} lg={2} key={index} >
                ...
                <Dialog onClose={collapsePhoto} open={expand} style={{ objectFit: 'contain' }}>
                    <img src={`http://127.0.0.1:8080/ipfs/${expandedPhoto}`} style={{ width: '100%', height: '70%' }} />
                </Dialog>
                <Fab id='floating-add-button' onClick={showDialog} size='small' color='primary' >
                <UploadIcon />
                </Fab>
